...
surfiL = nib.freesurfer.read_geometry('gradient_data/templates/S900.L.very_inflated_MSMAll.32k_fs_LR.surf') # JG_MOD
surfL = []
#surfL.append(np.array(surfmL[0]*0.3 + surfiL[0]*0.7))
#surfL.append(np.array(surfmL[0]*0.3 + surfiL[0]*0.7))
surfL.append(np.array(surfmL[0]*0.3 + surfiL[0]*0.7)) # JG_MOD
...
sulcL[cortL] = -1 * np.squeeze(nib.load('gradient_data/templates/S900.sulc_MSMAll.32k_fs_LR.dscalar.nii').dataobj)[:len(cortL)] # JG_MOD

#sulcR[cortR] = -1 * nib.load('gradient_data/templates/S900.sulc_MSMAll.32k_fs_LR.dscalar.nii').data[len(cortL)::] # JG_MOD
sulcR[cortR] = -1 * np.squeeze(nib.load('gradient_data/templates/S900.sulc_MSMAll.32k_fs_LR.dscalar.nii').dataobj)[len(cortL)::]
...
sulcL[np.setdiff1d(range(32492),cortL)] = -1 * np.squeeze(nib.load('gradient_data/templates/Q1-Q6_R440.sulc.32k_fs_LR.dscalar.nii').dataobj)[np.setdiff1d(range(32492),cortL)] # JG_MOD
...
sulcR[np.setdiff1d(range(32492),cortR)] = -1 * np.squeeze(nib.load('gradient_data/templates/Q1-Q6_R440.sulc.32k_fs_LR.dscalar.nii').dataobj)[32492+np.setdiff1d(range(32492),cortR)] # JG_MOD
...
# JG_MOD
# - need to come back to making this one work...
...
# color=np.array(c), alpha=0.8,
#                   ylim = [np.min(df['e0']) - 0.5, np.max(df['e0']) + 0.5],
#                   xlim=[np.min(df['e1']) - 0.5, np.max(df['e1']) + 0.5]) # , stat_func=None) # JG_MOD

# JG_MOD - does this stop the 'c' argument bug?
g.set_axis_labels('Gradient 2', 'Gradient 1')
